...
const TEST_CASES: Array<{ value: CborValue; expected: string }> = [
  // ###### SIMPLE ######
...
describe('encodeWithSelfDescribedTag', () => {
  it.each<{ value: CborValue; expected: string }>(TEST_CASES)(
    ...
    { timeout: 10_000 }, // some cases take more than 5 seconds to run
    ({ value, expected }) => {
      const result = encodeWithSelfDescribedTag(value);

      expect(bytesToHexString(result)).toEqual(`D9D9F7${expected}`);
    },
  );

  it('should throw if a positive value cannot fit within 8 bytes with self-described tag', () => {
    const value = 18_446_744_073_709_551_616n;
    const error = 'Value too large to encode: 18446744073709551616';

    expect(() => encodeWithSelfDescribedTag(value)).toThrow(error);
  });

  it('should throw if a negative value cannot fit within 8 bytes with self-described tag', () => {
    const value = -18_446_744_073_709_551_617n;
    const error = 'Value too large to encode: 18446744073709551616';
  ...
  describe('encode with replacer with self-described tag', () => {
    it('should handle objects', () => {
      const value = { a: 1, b: 2 };
      const replacer: Replacer = value =>
        typeof value === 'number' ? value * 2 : value;
      const result = encodeWithSelfDescribedTag(value, replacer);

      expect(bytesToHexString(result)).toEqual('D9D9F7A2616102616204'); // { "a": 2, "b": 4 } with self-described tag
    });
    ...
    it('should handle null and undefined values', () => {
      ...
      expect(bytesToHexString(result)).toEqual('D9D9F7A26161646E756C6C6162F7'); // { "a": "null", "b": undefined } with self-described tag
    });
    ...
    it('should handle nested objects', () => {
      ...
      expect(bytesToHexString(result)).toEqual('D9D9F7A16161A1616204'); // { "a": { "b": 4 } } with self-described tag
    });
    ...
    it('should handle arrays', () => {
      ...
      expect(bytesToHexString(result)).toEqual('D9D9F783020406'); // [2, 4, 6] with self-described tag
    });
    ...
    it('should handle objects with booleans', () => {
      ...
      expect(bytesToHexString(result)).toEqual('D9D9F7A26161F56162F4'); // { "a": true, "b": false } with self-described tag
    });
  });

  it('should encode concurrently with self-described tag', async () => {
    const values = [
      { a: 1, b: 2 },
      { c: 3, d: 4 },
      { e: 5, f: 6 },
    ...
    const results = await Promise.all(
      values.map(value => encodeWithSelfDescribedTag(value)),
    );
